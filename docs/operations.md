...
`zenith-model-feature-dock` is centered on ml utilities work in Haskell. The technical target is: Create a Haskell reference implementation for feature workflows, centered on graph analysis, node-edge fixtures, and cycle and reachability reports.
...
The score is deterministic. The current threshold is 168. Risk has a penalty of 5, latency has a penalty of 4, and weight contributes a bonus of 4.
